...
Y <- rbinom(1, size=1000, prob=0.42) ## Y ~ Binom(n, p) where n=1000, p=0.42
...
Y_10k_repeated_100 <- rbinom(100, size=10000, prob=0.42)

## Visualize the resulting distribution of % approve. 
## How does it compare to before?
tibble(approval=Y_10k_repeated_100) %>%
  mutate(approval=approval/10000) %>%
  ggplot(aes(x=approval)) +
  geom_histogram(bins = 100) +
  scale_x_continuous(limits = c(0.3, 0.5)) +
  labs(x="Prop Who Approve of Trump", y="Count") +
