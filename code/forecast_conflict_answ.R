...
length(unique(wardat$year))
length(unique(wardat$country))

## How many instances of civil war?
table(wardat$civilwar)
...
# Create an empty results data frame to collect results
...
fit_and_eval_forest(
  wardat_train, wardat_test, model_name = "Random Forest (Jungle)",
  formula = fearon_laitin_formula, num_trees = 500
)

# Check the predictors ----------------------------------------------------

mdl <- glm(fearon_laitin_formula, family = "binomial", 
           data = wardat_test)
summary(mdl)

tidy(mdl, exponentiate = TRUE) %>%
  arrange(desc(abs(estimate))) %>%
  ggplot(aes(y=term, xmin=estimate-1.96*std.error, x=estimate, xmax=estimate+1.96*std.error)) +
