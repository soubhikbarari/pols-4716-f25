...
balance <- transphobia %>%
  ...
  summarise(age = mean(age),
            female = mean(female),
            democrat = mean(democrat == 1),
            # just checking balance for one racial category for now
            race_white = mean(racename == "Caucasian"),
            therm_trans_t0 = mean(therm_trans_t0),
            voted_gen_12 = mean(voted_gen_12),
            voted_gen_14 = mean(voted_gen_14))
```
...
```{r visualize-balance}
balance %>%
  # this turns our data from 'wide' to 'long' to fits ggplot's expected format
  pivot_longer(names_to = "var", values_to = "mean", -treat_ind) %>%
  # create a nicer label
  mutate(treatment = if_else(treat_ind==1, "Treat", "Control")) %>%
  ggplot(aes(x=var, y=mean, group=treatment, fill=treatment)) +
...
```{r sate-three-days-in-base-r}
# Base R version:
mean(transphobia$therm_trans_t1[transphobia$treat_ind==1])-
  mean(transphobia$therm_trans_t1[transphobia$treat_ind==0])
```

```{r sate-three-days-in-dplyr}
# Tidy versions:

## Two steps
treat_mean <- transphobia %>%
  filter(treat_ind == 1) %>%
  summarise(therm_trans_t1_mean = mean(therm_trans_t1))

control_mean <- transphobia %>%
  filter(treat_ind == 0) %>%
  summarise(therm_trans_t1_mean = mean(therm_trans_t1))
...
transphobia %>%
  summarise(
    diff_in_means = mean(therm_trans_t1[treat_ind == 1]) -
                    mean(therm_trans_t1[treat_ind == 0])
  )

## All in one chain without indexing using the base R diff() function
transphobia %>%
  # Calculate individual means by treatment group (Y_treat, Y_control)
  group_by(treat_ind) %>%
  summarise(m = mean(therm_trans_t1)) %>%
  # Collapse down to a single difference
  summarise(diff_in_means = diff(m))
```
...
```{r sate-three-weeks-base-r}
mean(transphobia$therm_trans_t2[transphobia$treat_ind==1])-
  mean(transphobia$therm_trans_t2[transphobia$treat_ind==0])
```
