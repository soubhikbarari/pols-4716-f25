...
Brainstorm what a *policy-relevant typology of countries* might look like:
...
- Run the next block (transform-vars). 
  - What is that doing?
...
cluster_vars <- c(
  ...
  'agriculture_land_pct', 'unemployment_rate', 
  'industry_value_added_pct', 'household_consumption_pct', 
  ...
  ## Health & social
  'health_expenditure_pc', 'health_expenditure_gdp', 
  'hospital_beds_per_1k', 'maternal_mortality', 
  'access_to_safe_water',
  
  ## Education
  'primary_enrollment', 'secondary_enrollment', 
  'tertiary_enrollment', 'adult_literacy_rate', 
  ...
  'air_passengers', 'co2_emissions_pc'
)
```

```{r transform-vars}
# This standardizes all of our variables into Z-scores so that they 
# are more easily comparable when we visualize or analyze them later
wdi_std <- wdi %>%
  mutate(across(
    all_of(cluster_vars),
...
TASK 2b
=======
Now take a closer look at the loadings to interpret what the components represent. 
Run the following two plots.
...
wdi_reduced %>%
  plot_pca_2d(labels = "country", with_loadings = TRUE)
```
...
Before doing this, make a prediction: what might the loss (y axis)
...
<!--
Consider tweaking the function arguments here for a better display.
-->
...
Unique countries in cluster:

```{r look-at-countries-in-each-cluster-v1, echo=TRUE}
unique(wdi_clustered$country[wdi_clustered$cluster == 1])

unique(wdi_clustered$country[wdi_clustered$cluster == 2])

unique(wdi_clustered$country[wdi_clustered$cluster == 3])

unique(wdi_clustered$country[wdi_clustered$cluster == 4])
```

Dataframe of countries in each cluster:

```{r look-at-countries-in-each-cluster-v2, echo=TRUE}
wdi_clustered %>%
  group_by(cluster) %>%
...
Table of regions represented in each cluster:

```{r look-at-regions-in-each-cluster-v3, echo=TRUE}
table(wdi_clustered$region, wdi_clustered$cluster)
```
...
TASK 5a
=======
Now we want to interpret our cluster outputs by looking at how key variables are
...
These quick cross-tabulations can help us form hypotheses about what each cluster 
represents *before* diving into any modelling.
-->

```{r country-tables}
table(wdi_clustered$cluster, wdi_clustered$gdp_per_capita > 0)
table(wdi_clustered$cluster, wdi_clustered$exports_gdp_pct > 0)
table(wdi_clustered$cluster, wdi_clustered$infant_mortality > 0)
table(wdi_clustered$cluster, wdi_clustered$unemployment_rate > 0)
table(wdi_clustered$cluster, wdi_clustered$co2_emissions_pc > 0)
```
...
The goal in this step is to identify **significant (in both the statistical and 
scientific sense)** predictors for each cluster.
...
```{r regression-clus}
# predictors <- c() # TODO
what_predicts_membership_in_cluster(clus = 1,
                                    predictors = predictors, 
                                    data = wdi_clustered)
what_predicts_membership_in_cluster(clus = 2,
                                    predictors = predictors, 
                                    data = wdi_clustered)
what_predicts_membership_in_cluster(clus = 3,
                                    predictors = predictors, 
                                    data = wdi_clustered)
```
...
- Revisit the goals you outlined at the beginning. How might you refine your 
approach to move closer to those goals? Consider these in your discussion and notes: 
